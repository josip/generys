...
Object do(
  curlyBrackets := method(
    ...
    if(call message argAt(0) next ?name != ":",
      obj = call message argsEvaluatedIn(call sender)
    ,
      obj = Map clone
      # FIXME: The order is not preserved
      call message arguments foreach(arg,
        obj atPut(arg name, arg next next doInContext(call sender)))
    )
...
# Works on List, Map and Sequence
# Usage:
#   anList[0] == anList at(0)
#   anMap["key"] == anMap at("key")
...
Directory proto do(
  doFiles := method(
    self fileNames foreach(fileName, fileName containsSeq(".io") ifTrue(doFile(path .. fileName))))
